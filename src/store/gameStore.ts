...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  makeAIMove: () => {
    ...
    const actions: AIActions = {
      takeCard: (id) => get().takeCard(id),
      takeAllShips: () => get().takeAllShips(),
      sellCards: (ids) => get().sellCards(ids),
      exchangeCards: (hand, mkt) => get().exchangeCards(hand, mkt),
      pirateRaid: (id) => get().pirateRaid(id),
    };

    computeAIMove(view, actions);
  },
